...
class Mosaic():
    def __init__(self, input_filename, output_filename, bg):
    ...
    def run(self):
        ...
        # 先将图片映射好，方便操作
        if os.path.exists('./data/a_hash_mapping'):
            f1 = open('./data/avg_rgb_mapping', 'rb')
            f2 = open('./data/a_hash_mapping', 'rb')
            f3 = open('./data/np_mapping', 'rb')
            ...
            f2.close()
            f3.close()
            # calc_bgr_mapping = pickle.load(open('./data/calc_bgr_mapping', 'rb'))
        else:
            ...
            for k, file in tqdm(enumerate(os.listdir(self.bg)), desc='mapping映射中'):
                ...
                # 平均色值
                avg_rgb_mapping[file] = np.array(
                    [np.mean(np_mapping[file][:, :, 0]), np.mean(np_mapping[file][:, :, 1]),
                ...
                f1 = open('./data/avg_rgb_mapping', 'wb')
                f2 = open('./data/a_hash_mapping', 'wb')
                f3 = open('./data/np_mapping', 'wb')
        ...
        # 将图片扩大为新图片的大小
        # 将图片转成jpg格式
        new_image = Image.open(self.input_filename).resize((int(new_image_w), int(new_image_h)), Image.ANTIALIAS)
        ...
        for i in tqdm(range(w), desc='合成图片中'):
            for j in range(h):
                ...
                item = self.hash_img_code(np.array(Image.fromarray(piece).convert('L').resize((8, 8), Image.ANTIALIAS)))
                selection = [(key, np.equal(item, a_hash_mapping[key]).mean()) for key, _ in selection[-10:]]

                # 将最同样的那一张图贴到原图里面
                new_image[j * mini_image_h:(j + 1) * mini_image_h, i * mini_image_w:(i + 1) * mini_image_w, :] = \
                    np_mapping[
...
if __name__ == '__main__':
    mosaic = Mosaic('./demo/input.jpg', './output.jpg', './bg')
    mosaic.run()
